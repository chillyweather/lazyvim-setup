return {
  -- Local luca theme
  {
    ...
    name = "luca-theme",
    lazy = false,
    priority = 1002, -- Higher than themery
    config = function()
      -- Create proper lua theme with correct colors
      local colors = {
        bg = "#2b3e50",
        fg = "#f8f8f2",
        current_line = "#243443",
      ...
      -- Create a new colorscheme file that uses our highlighting function
      local colors_file = vim.fn.stdpath("config") .. "/colors/luca.vim"
      local file_content = [[
" Luca Theme
...
EOF
      ...
      vim.fn.mkdir(vim.fn.fnamemodify(colors_file, ":h"), "p")
      ...
      -- Call our setup function to set the theme
      setup_theme()

      -- Register a ColorScheme autocmd to ensure our theme always applies
      vim.api.nvim_create_autocmd("ColorScheme", {
        pattern = "luca",
        callback = setup_theme,
        group = vim.api.nvim_create_augroup("LucaTheme", { clear = true }),
      })
    end,
  },
}